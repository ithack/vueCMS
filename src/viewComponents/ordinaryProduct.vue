<template>
  <div class="component ordinary-product" :style="node.styl" :component-name="node.name">
    <ul class="product_list">
      <li v-for="(item,index) in productList" v-$log :taglog="node.id+'|'+node.name+'|k'+index+'|'+node.other.pid+'|itemid-'+index+'|itemtype1'" data-price-area="need" @click.stop="itemLinkUrl(item.skuid)">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      itemLinkUrl(skuId){
        window.location.href=link().mItem+'/ware/view.action?wareId='+skuId
      }
    },
  }
</script>
