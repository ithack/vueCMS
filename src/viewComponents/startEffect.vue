<template>
  <div class="component startEffect"  :component-name="node.name" v-show="mVisible">
          <div class="container3">
              <div class="btn1 btn" v-if="node.other.isStyle == '0'" :style="{'background-color':node.styl.backgroundColor,'color':node.styl.color,'border-color':node.styl.borderColor}" @click="skipFun1">
                  跳转{{mTime}}秒
              </div>
              <div class="btn2 btn" v-else-if="node.other.isStyle == '1'" :style="{'background-color':node.styl.backgroundColor,'color':node.styl.color,'border-color':node.styl.borderColor}" @click="skipFun1">
                   跳转{{mTime}}秒
              </div>
              ...
              <img :src="node.other.pictureVideo" alt="" :style="node.styl">
          </div>
  </div>
</template>
